...
contract HelpConfig is Script {
    ...
    mapping(uint256 => address) public priceFeedAddress;
    NetworkConfig public i_activeNetworkConfig;
    MockV3Aggregator mockPriceFeed;

    constructor() {
        priceFeedAddress[11155111] = 0x694AA1769357215DE4FAC081bf1f309aDC325306; // Sepolia ETH/USD
        priceFeedAddress[1] = 0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419; // Mainnet ETH/USD
        priceFeedAddress[421614] = 0xd30e2101a97dcbAeBCBC04F14C3f624E67A35165; // Arbitrum Sepolia ETH/USD
        priceFeedAddress[80002] = 0xF0d50568e3A7e8259E16663972b11910F89BD8e7; // Polygon Amoy ETH/USD
        priceFeedAddress[11155420] = 0x61Ec26aA57019C486B10502285c5A3D4A4750AD7; // Optimism Sepolia ETH/USD
        priceFeedAddress[84532] = 0xE2E1CECaF186D44A4B01f46D6A7EcaE2B89c8076; // Base Sepolia ETH/USD
        priceFeedAddress[300] = 0xfEefF7c3fB57d18C5C6Cdd71e45D2D0b4F9377bF; // ZkSync Sepolia ETH/USD

        // i_activeNetworkConfig = getActiveNetworkConfigWithMapping();
    ...
    function getOrCreateAnvilEthConfig() private returns (NetworkConfig memory) {
        ...
        // // Deploy mocks
        // vm.startBroadcast();
        // MockV3Aggregator mockpriceFeedAddress = new MockV3Aggregator(18, 2000e8);
        // vm.stopBroadcast();

        // anvilConfig = NetworkConfig({priceFeedAddress: address(mockpriceFeedAddress)});
        // return anvilConfig;
    }

    function getActiveNetworkConfigWithMapping() public returns (NetworkConfig memory) {
        NetworkConfig memory activeNetworkConfig;
        // if (activeNetworkConfig.priceFeedAddress != address(0)) return activeNetworkConfig;
        ...
        if (priceFeedAddress[block.chainid] == address(0)) {
            revert NetworkNotSupported(block.chainid);
        }

        console.log("Using price feed address: ", priceFeedAddress[block.chainid]);
        activeNetworkConfig = NetworkConfig({priceFeedAddress: priceFeedAddress[block.chainid]});
        return activeNetworkConfig;
    }
    ...
    function getActiveNetworkConfig() public returns (NetworkConfig memory) {
        NetworkConfig memory activeNetworkConfig;
        // NetworkConfig memory activeNetworkConfig; // = NetworkConfig({priceFeedAddress: address(0)});
        if (block.chainid == 11155111) {
            ...
            activeNetworkConfig = getOptimismSepoliaEthConfig();
        } else if (block.chainid == 84532) {
            activeNetworkConfig = getBaseSepoliaEthConfig();
        } else if (block.chainid == 80002) {
            activeNetworkConfig = getPolygonAmoyEthConfig();
